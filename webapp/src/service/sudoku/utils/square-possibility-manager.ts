...
export class SquarePossibilityManager {
  ...
  calculatePossibilities({ value, grid, row, column }: Square): Set<number> {
    ...
    const possibilities = new Set<number>([1, 2, 3, 4, 5, 6, 7, 8, 9]);
    const removePossibility = (square: Square) =>
      possibilities.delete(square.value);
  ...
  removeValueFromGroupPossibilities({
    value,
    grid,
    row,
    column,
